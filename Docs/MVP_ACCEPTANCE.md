...
| 隐私同意与数据发送门 | PARTIAL | 法律入口存在，但运行时未提供明确接受/拒绝；分析与 Development 登录需在同意前关闭。 |
| 任务/留存语义 | PARTIAL | 当前“一次性领取”不是真正每日任务；需实现 UTC 日边界或诚实改为一次性成长任务。 |
...
V0.1 已进入 RC-candidate 开发后段，但 **MVP 尚未完成，GATE 4 保持 OPEN**。当前执行顺序：先收口可执行 P1，再在目标 Unity 会话可用时运行完整 EditMode/PlayMode，生成同源 APK/AAB，完成九页竖屏与物理真机 10/60 分钟验收。历史 109/26、旧产物、模拟器截图或旧真机豁免均不得用于放行。
