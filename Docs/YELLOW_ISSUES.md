...
| Y-009 | Android 签名 | `2bee3ac` 历史 RC APK/AAB 使用 Unity 默认 Android 测试签名，且已不对应当前源码 | 仅作历史安装参考，不作为 `bffd195` 当前 RC 或商店产物 | 当前源码全绿后重建；商店上传前配置独立 upload/release keystore 并复验 |
...
| Y-011 | 测试覆盖 | UnityWebRequest 客户端到真实服务端的同进程/设备联调证据尚未覆盖 | 真实 Kestrel HTTP 39/39 已覆盖服务端路由、鉴权、profile、inventory、battle 成功/拒绝/幂等契约；109/26 是 `2bee3ac` 历史证据，当前 Unity 未运行 | 对外启用 Development 在线入口或真机联调时补 UnityWebRequest 端到端证据，不阻塞离线商业验证 |
...
| Y-013 | Development 在线境界 | 后端突破仍消费等级经验残值与软币，尚未与离线累计修为、破境石及 pending→Boss 渡劫语义完全一致 | V0.1 RC 默认离线循环；在线入口仅 Development 可见 | `SERVER-REALM-XP-001` 先分离累计修为，再评估材料和渡劫合同迁移 |
